...
with st.sidebar:
      ...
      MAX_HISTORY_LENGTH = st.number_input(
                            ...
                            value=5,
                            step=1)
      medical_perscribtion = st.sidebar.file_uploader("Please Upload a medical document") 
      medical_image  = st.sidebar.file_uploader("Please upload a medical image",type=("png","jpeg","webp"))
      prompt = st.sidebar.text_input("Please your prompt for process the medical image.")
      show_button=st.button("Show The Comment")  
      create_database = st.button('Create a VectorDatabase')
      load_database = st.button('Load a VectorDatabase')
      save_database = st.button('Save a VectorDatabase Local')
...
MAX_MIN_RETRIES = 10
def get_gpt_image_description(image_url,prompt,model):
  """
...
def get_image_description(image_file,prompt,model="gpt-4o"):
  """
  Open AI model takes the image and model and get the description for the image
  """
  base64_image = encode(image_file)
  return get_gpt_image_description(image_url=f"data:image/jpeg:base64,{base64_image}",prompt=prompt,model=model)
...
def main():
    ...
    st.title("Multi-modal Medical Assistant unofficial")
    # Initialize OpenAI 
    ...
    try:
            if medical_image and show_button and prompt:
                st.image(medical_image)
                if st.session_state.desc ==None:
                    description=get_image_description(medical_image,prompt,model="gpt-4o")
                    doc=Document(page_content=description)
                    st.session_state.desc = doc
                else:
                    st.image(medical_image)
                    st.write(st.session_state.desc.page_content)     

    except:
             st.write("Please check the prompt or upload the image or press the show comment")                   
                     
    if create_database and medical_perscribtion:
        with st.spinner("Analyzing the Document and Image..."):
             ...
             doc = document_layout_analysis_azure(medical_perscribtion)
             
        with st.spinner("Semantic Chunking..."):
             if st.session_state.desc is not None:
                  print(type(st.session_state.desc))
                  chunks_desc = fixed_chunking(st.session_state.desc)  
        ...
        with st.spinner("Creating Vector database based on Semantic Chunking..."):
                try:  
                    if st.session_state.crtdata==None:
                        ...
                        vec_db=create_vector_database_azure(chunks_desc)
                    ...
                    st.sidebar.write("Database didn't created successfully please upload your files")   
    if save_database:
        try:      
